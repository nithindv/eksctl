...
func getIAMIdentityMappingCmd(g *cmdutils.Grouping) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			if err := doGetIAMIdentityMapping(p, cfg, roleFlag, cmdutils.GetNameArg(args)); err != nil {
				logger.Critical("%s\n", err.Error())
				os.Exit(1)
			}
		},
...
func doGetIAMIdentityMapping(p *api.ProviderConfig, cfg *api.ClusterConfig, roleFlag, roleArg string) error {
	ctl := eks.New(p, cfg)
	...
	if roleFilter != "" {
		if role, ok := roles.Get(roleFilter); ok {
			roles = authconfigmap.MapRoles{role}
		} else {
			roles = nil
		}
	}
